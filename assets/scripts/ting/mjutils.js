function checkTingPai(seatData,begin,end){
	var laiZi = -1;
	if(seatData.game && seatData.game.magic>-1){
		laiZi = seatData.game.magic;
	}
	...
	for(var i = begin; i < end; ++i){
        ...
        //     var index =  seatData.holds[j];
        //     var c =  seatData.countMap[index];
        //     if(c == null) c = 0;
        //     seatData.countMap[index] = c + 1;
        // }
		// if(old == null){
		// 	old = 0;
		...
		// }else{
		// 	if (i != laiZi) {
				var hu = require('./huCheck');
				hu.checkPingHu (holds, i,laiZi,function callback(isPingHu,allKeZi,shunZis,keZis,duiZi,laiZiReplacePais,replacedpais){
				//hu.checkPingHu(seatData,laiZi,function callback(ret,allKeZi,shunZis,keZis,duiZi,laiZiReplacePais,replacedpais){
					if(isPingHu){
						seatData.tingMap[i] = {
							pattern:"normal",
							fan:0,
