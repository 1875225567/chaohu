cc.Class({
    ...
    getWanfa:function(){
        ...
        if(conf){
            ...
            if(conf.facaidaikao == 1){
                strArr.push("发财带靠、");
            }
            if(conf.duanyaojiu == 1){
                strArr.push("断幺九、");
            }
            if(conf.shiyao == 1){
                strArr.push("十幺、");
            ...
            var reg = new RegExp("、");
            if(strArr[num]){
                strArr[num] = strArr[num].replace(reg,"。");
            }
            return strArr.join(" ");
        }
        return "";
    ...
    initHandlers:function(){
        ...
        cc.vv.net.addHandler("login_result",function(data){
            ...
            if(tableclothData.roomId !== data.roomid){
                tableclothData.roomId = data.roomid;
                tableclothData.newRoom = true;
                tableclothData.color = "green";
        ...
        cc.vv.net.addHandler("game_over_push",function(data){
            ...
            setTimeout(() => {
                ...
                for(var i = 0; i <  self.seats.length; ++i){
                    self.seats[i].score = results.length == 0? 0:results[i].surplusscore;
                }
                if(results){
        ...
        cc.vv.net.addHandler("huang_notify_push",function(data){
            console.log('huang_notify_push');
            console.log(data);
            var userId = data.userid;
            var si = self.getSeatIndexByID(userId);
            self.doHuang(si);
        ...
        cc.vv.net.addHandler("dissolve_notice_push",function(data){
            ...
            self.dispatchEvent("dissolve_notice");
        });
        ...
        //增志子选择
        cc.vv.net.addHandler('game_zengzhizi_push',function (data) {
            console.log("game_zengzhizi");
            self.dispatchEvent('game_zengzhizi',data);
        });

        cc.vv.net.addHandler('game_zengzhizi_finish_push',function (data) {
            self.dispatchEvent('game_zengzhizi_finish',data);
        });
    },
    ...
    doChupai:function(seatIndex,pai){
        ...
        // var idx = null;
        // if(seatData.holds){
        //     idx = seatData.holds.indexOf(pai);
        //     seatData.holds.splice(idx,1);
        // }
        this.dispatchEvent('game_chupai_notify',{seatIndex:seatIndex,pai:pai});
    },
    ...
    doHuang:function(seatIndex){
        var seatData = this.seats[seatIndex];
        this.dispatchEvent('huang_notify',seatData);
    },
